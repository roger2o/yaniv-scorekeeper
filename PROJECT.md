...
- **In build — Phases 0 & 1 complete (2026-06-02).** React + Vite + TypeScript scaffold in place (Vitest test runner, `vite-plugin-pwa` installed but not yet wired, dev server binds `--host`). The pure scoring engine is built under `src/engine/` as a single `recompute(history, settings)` function that derives all game state from the round-history list, with 23 passing developer unit tests across every locked rule and key boundary. Local git repo initialised with an initial commit; no remote yet (Wells owns remote/deploy).
- **Next:** Phase 2 — full adversarial engine test suite (Bugsy) and engine review (Holmes) before any UI.
