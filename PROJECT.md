...
- **Online landing page** — the public URL the app is shared by. Before installing, a visitor sees what the app is, why to install it (works offline, full-screen, no app store), **step-by-step install instructions for both Android and iPhone/iPad**, and the full how-to-play / how-to-keep-score guide. This page is how every device gets its own copy (see Distribution model).
- **In-app Help / How-to screen** — the same how-to guide, reachable any time from inside the installed app via a Help control, minus the install steps. The how-to copy is authored once and shared between the landing page and the in-app screen so the rules can never drift apart. (Drafted copy: `docs/landing-and-help-content.md`.)
...
- **Landing page + in-app Help, sharing one how-to** — the app ships with a public landing page (on the same static URL it's distributed from) carrying the "why install", Android and iPhone/iPad install steps, and the how-to guide; and an in-app Help screen carrying the same how-to. The how-to copy is written **once** and reused in both places. *Why:* the audience is non-technical casual players who need install hand-holding (and iPhone users specifically must be told to use Safari, since other browsers can't install a PWA on iOS), and the in-app copy means a player can re-check the rules mid-game with no signal. Single-source copy prevents the landing page and the app from disagreeing on the rules. Drafted copy lives in `docs/landing-and-help-content.md`. Consistent with the URL/PWA distribution model — the landing page *is* the install entry point each device uses. (Decision 2026-06-02.)
