...
- **In build — Phases 0, 1, 2 & 3 complete (2026-06-02).** React + Vite + TypeScript scaffold in place (Vitest test runner, `vite-plugin-pwa` installed but not yet wired, dev server binds `--host`). The pure scoring engine is built under `src/engine/` as a single `recompute(history, settings)` function that derives all game state from the round-history list. Bugsy's adversarial suite and Holmes's engine review are done; Holmes's review fixes are applied (see below). **Phase 3 adds the application state layer and crash-safe persistence under `src/state/`, plus a thin app shell (`src/screens/`) that switches setup → play → end.** Full suite now passes **118 unit tests** (82 engine + 36 state/persistence) with a clean TypeScript + Vite build. Local git repo only; no remote yet (Wells owns remote/deploy).
