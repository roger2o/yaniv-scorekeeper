...
- [ ] Verify install + full offline on a real device in airplane mode (test against HTTPS preview, not LAN dev server) — *Wells; SW + install only work over HTTPS/localhost, so this happens on the Netlify preview, not the WSL2 LAN dev server*
...
## Current Focus
**Landing page built — the pre-install browser entry point (Phase 10a, Turing).** Opening the app's URL in a normal browser tab now shows a themed landing page before anything is installed: hero + one-line summary, "Why install?" (offline / full-screen / no app store), step-by-step **Install on Android** (Chrome) and **Install on iPhone/iPad** (leads with the Safari-only gotcha), then **both** guides — **How to Use** and **How to Play** — rendered from the SAME single-source blocks the in-app Help uses (`src/content/helpContent.tsx`), so the two surfaces can never drift. A **"Start scoring now"** button (top and foot) drops a visitor straight into the app in the browser without installing. **Install/standalone gate:** an installed/standalone launch (`display-mode: standalone`, or iOS `navigator.standalone`) skips the landing entirely; a browser visitor sees it, and "Start scoring now" dismisses it for the rest of the visit (sessionStorage) while a genuinely fresh visit can still see it. Themed via the existing token layer (Felt & Chips / Party Arcade, with the theme toggle offered on the landing too); mobile-first at 375px; accessibility floor inherited (≥56px primary action, focus-visible, reduced-motion, semantic landmarks/headings, AA contrast). Terminology verified — never "deal/dealer" (the one "dealt 5 cards" is the rules-of-the-physical-game copy in How to Play, the allowed exception). The shared guide prose styling was lifted out of `HelpDialog.css` into `src/content/helpProse.css` and imported by both surfaces so the guides look identical and the CSS is never duplicated. **Full suite green at 367 tests** (10 new landing tests: browser-renders vs standalone-bypass, both guides render and are byte-for-byte the shared source, Android + iOS install sections incl. the Safari gotcha, "Start scoring now" enters the app, dismissal-persists-within-visit, no "dealer"), clean `tsc -b` + production `vite build` (the landing is part of the precached shell, so it works offline too). The protected PWA files (`vite.config.ts`, `src/pwa/*`, `src/main.tsx` SW registration, the manifest, index.html PWA bits) were NOT touched. Branch `yaniv-themed-ui`; local git only, no remote. *Handoffs: Twiggy (UX of the landing — install-step clarity, the Safari gotcha prominence, on-device look in both themes) and Holmes (code review — the gate logic + the prose-CSS extraction).*

**PWA wiring complete — the app is now installable + fully offline (Phase 10 build items, Turing).** `vite-plugin-pwa` is wired in `vite.config.ts` with an **autoUpdate** strategy (skipWaiting + clientsClaim; `registerSW({ immediate: true })` in `main.tsx`) so a refreshed device silently adopts the newest build — the deliberate guard against a scoring bug living forever on an un-patchable offline device. The production build generates `dist/sw.js` + `dist/manifest.webmanifest` and **precaches the full app shell (22 entries, ~321 KiB)** — html/js/css/icons/manifest/fonts — so first-load-then-offline is reliable; the HTML navigation is served **NetworkFirst** (3s timeout → cached index) so a new bundle is discovered on the next online load while a fully-offline launch still works. **Manifest** (`src/pwa/manifest.ts`, imported by the config): name "Yaniv Scorekeeper", short_name "Yaniv", standalone, portrait, scope `/`, lang `en`, theme/background = Felt & Chips felt-green `#1f4a3d`. **Icons:** 192/512/512-maskable + apple-touch-icon (180, opaque) + favicon — **PLACEHOLDER card-table art** (felt tile, brass rim, two ivory cards with a red diamond + dark club) generated by the zero-dependency `scripts/generate-icons.mjs`; an editable `public/icon.svg` is the handoff for final art. **Fonts self-hosted locally** (`public/fonts/nunito-latin.woff2` + `baloo2-latin.woff2`, OFL, ~72 KB, `@font-face` in `src/theme/fonts.css`) so the display faces (Nunito / Baloo 2) work offline — no Google Fonts CDN. **Full suite green at 357 tests** (3 new manifest-shape tests), clean `tsc -b` + production `vite build`. autoUpdate needs no "update ready" UI, so none was built. *Caveat: SW + install only work over HTTPS/localhost — true install/offline verification must run on the Netlify HTTPS preview (Wells), not the WSL2 LAN dev server. Handoffs: Wells (PWA/deploy-readiness review + install/offline verification on the preview) and Holmes (quick config code review).*
...
**Still open:** landing page (Phase 10a — should import the same `helpContent.tsx` blocks), the on-device install/offline verification (Phase 10 last item — Wells, on the HTTPS preview), accessibility/device pass (11), quality gate (12), deploy (13). Turing also still owes Holmes the EndGameScreen FIX 4 re-review.
...
**No longer deferred:** the round-by-round history is now delivered as the vertical scoresheet (above), so the separate read-only history panel is dropped. Still ahead: the landing page (Phase 10a), accessibility/legibility device pass (Phase 11), quality-gate/security sweep (Phase 12), and deploy (Phase 13).

**Next:** Holmes config review of the PWA wiring; Wells PWA/deploy-readiness review + install/offline verification on the Netlify HTTPS preview; Twiggy UX review of the two themes + two views; Bugsy functional tests on a device; then the landing page (10a) and Phases 11–13.
